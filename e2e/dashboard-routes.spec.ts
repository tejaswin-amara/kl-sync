...
test.describe('Comprehensive 11-Route E2E Browser Verification', () => {
  ...
  test('Route 3: /dashboard/timetable (Student Timetable) renders matrix grid & list views', async ({ page }) => {
    ...
    await expect(page.locator('#main-content').getByRole('heading', { name: /Student Timetable/i })).toBeVisible();
  ...
  test('Route 5: /dashboard/marks (Internal Marks) renders assessment scores', async ({ page }) => {
    ...
    await expect(page.locator('#main-content').getByRole('heading', { name: /Marks & Grades/i })).toBeVisible();
  ...
  test('Route 6: /dashboard/profile (Student Profile) renders student info & courses', async ({ page }) => {
    ...
    await expect(page.locator('#main-content').getByRole('heading', { name: /Profile/i })).toBeVisible();
  ...
  test('Route 7: /dashboard/fee (Fee Details) renders fee balance & payment status', async ({ page }) => {
    ...
    await expect(page.locator('#main-content').getByRole('heading', { name: /Fee Details/i })).toBeVisible();
    await expect(page.getByText(/Total Pending/i)).toBeVisible();
    await expect(page.getByText(/Total Paid/i)).toBeVisible();
    expect(consoleErrors).toEqual([]);
  });

  test('Route 8: /dashboard/tools (Student Tools) renders calculators & tools', async ({ page }) => {
    await page.goto('/dashboard/tools');

    await expect(page.locator('#main-content').getByRole('heading', { name: /Tools & Calculators/i })).toBeVisible();
  ...
  test('Route 9: /dashboard/circulars (Circulars) renders announcements table', async ({ page }) => {
    await page.goto('/dashboard/circulars');

    await expect(page.locator('#main-content').getByRole('heading', { name: /Circulars/i })).toBeVisible();
    expect(consoleErrors).toEqual([]);
  });

  test('Route 10: /dashboard/hostels (Hostel Info) renders hostel allocation data', async ({ page }) => {
    await page.goto('/dashboard/hostels');

    await expect(page.locator('#main-content').getByRole('heading', { name: /Hostel Information/i })).toBeVisible();
    expect(consoleErrors).toEqual([]);
  });

  test('Route 11: /dashboard/library (Library) renders library records', async ({ page }) => {
    await page.goto('/dashboard/library');

    await expect(page.locator('#main-content').getByRole('heading', { name: /Library/i })).toBeVisible();
    expect(consoleErrors).toEqual([]);
  });

  test('Route 12: /dashboard/exam-seating (Exam Seating) renders seating data', async ({ page }) => {
    await page.goto('/dashboard/exam-seating');

    await expect(page.locator('#main-content').getByRole('heading', { name: /Exam Seating/i })).toBeVisible();
    expect(consoleErrors).toEqual([]);
  });
});
