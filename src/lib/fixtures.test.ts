...
test('fixtures exports all 9 expected fallback datasets', () => {
  ...
  assert.ok(Array.isArray(DEMO_TIMETABLE_RAW), 'DEMO_TIMETABLE_RAW must be an array');
  ...
  assert.ok(typeof DEMO_CAPTCHA_SVG === 'string', 'DEMO_CAPTCHA_SVG must be a string');
  assert.ok(DEMO_CAPTCHA_SVG.startsWith('data:image/svg+xml;base64,'), 'DEMO_CAPTCHA_SVG must be data URI');
