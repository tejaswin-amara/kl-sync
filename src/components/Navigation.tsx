...
function ProfileAvatar({ user, className = '', size = 'md' }: { user: { id: string; initials: string; photoUrl: string }; className?: string; size?: 'sm' | 'md' }) {
  const dimensions = size === 'sm' ? 'h-8 w-8 text-[10px]' : 'h-10 w-10 text-xs';
  const photoSrc = useMemo(() => {
    if (!user.photoUrl) return `/api/fetch-photo?id=${encodeURIComponent(user.id)}`;
    if (/^data:image\/(png|jpeg|jpg|webp|gif);base64,[A-Za-z0-9+/=]+$/i.test(user.photoUrl)) return user.photoUrl;
...
export default function Navigation({ children }: { children: React.ReactNode }) {
  ...
  const pageTitle = allNavItems.find((item) => item.href === pathname)?.label || allNavItems.find((item) => item.href !== '/dashboard' && pathname.startsWith(item.href))?.label || 'Overview';
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      const initials = name === 'Student' ? 'ST' : name.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase();
      setUser({ name, initials, id, photoUrl: cachedPhoto });
      if (!cachedName) {
        fetch('/api/erp-proxy/profile').then((response) => response.json()).then((data) => {
          const profile = data.profile || data.data;
          if (data.success && profile?.name) {
            localStorage.setItem('kl_student_name', profile.name);
            localStorage.setItem('kl_student_profile', JSON.stringify(profile));
            if (profile.photoUrl) localStorage.setItem('kl_student_photo', profile.photoUrl);
            setUser({ name: profile.name, initials: profile.name.split(' ').map((part: string) => part[0]).join('').slice(0, 2).toUpperCase(), id, photoUrl: profile.photoUrl || '' });
          }
        }).catch(console.warn);
      }
      if (localStorage.getItem('kl_sidebar_collapsed') === 'true') setCollapsed(true);
    });
  }, []);
  ...
  const renderNavLink = (item: (typeof allNavItems)[number], compact = false, closeDrawer?: () => void) => {
    ...
    return (
      <Link
        ...
        onClick={() => { triggerHaptic('selection'); closeDrawer?.(); }}
      ...
      >
        <Icon className={`h-[18px] w-[18px] shrink-0 ${active ? 'text-primary-foreground' : 'text-muted-foreground group-hover:text-primary'}`} />
    ...
    <div className="relative h-[100dvh] min-h-[100dvh] overflow-hidden bg-background text-foreground">
      <header className="apple-chrome fixed inset-x-0 top-0 z-40 flex h-[--header-height] items-center justify-between border-b px-4 lg:hidden">
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setComplianceOpen(true)} className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-full text-success hover:bg-surface-2" aria-label="Compliance and Privacy"><ShieldCheck className="h-5 w-5" /></button>
          <LanguageSelector />
          <Link href="/dashboard/profile" aria-label="Student profile" className="flex min-h-[44px] min-w-[44px] items-center justify-center"><ProfileAvatar user={user} size="sm" /></Link>
        </div>
      </header>
      ...
      <aside className={`apple-chrome fixed inset-y-0 left-0 z-30 hidden flex-col border-r transition-[width] duration-[--duration-normal] lg:flex ${collapsed ? 'w-[--sidebar-collapsed]' : 'w-[--sidebar-width]'}`}>
        <div className="flex h-[--header-height] items-center justify-between border-b px-4">
          ...
          {!collapsed && <button onClick={toggleCollapse} className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-full text-muted-foreground hover:bg-surface-2" aria-label="Collapse sidebar"><ChevronLeft className="h-4 w-4" /></button>}
        </div>
        <nav className="custom-scrollbar flex-1 space-y-1 overflow-y-auto px-2 py-5">
          {!collapsed && <p className="caption-label px-3 pb-3 text-muted-foreground">Workspace</p>}
      ...
      <nav className="apple-chrome-bottom fixed inset-x-0 bottom-0 z-40 lg:hidden" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        <div className="flex h-[--bottom-bar-height] items-center justify-around">{bottomItems.map((item) => { const active = isActive(item.href); const Icon = item.icon; return <Link key={item.href} href={item.href} aria-current={active ? 'page' : undefined} onClick={() => triggerHaptic('selection')} className={`flex h-full flex-1 flex-col items-center justify-center gap-1 text-[10px] font-semibold ${active ? 'text-primary' : 'text-muted-foreground'}`}><Icon className={`h-5 w-5 ${active ? 'scale-110' : ''}`} /><span>{item.label.split(' ')[0]}</span></Link>; })}<button onClick={() => { triggerHaptic('light'); setMoreOpen((current) => !current); }} aria-expanded={moreOpen} className={`flex h-full flex-1 flex-col items-center justify-center gap-1 text-[10px] font-semibold ${moreOpen || overflowItems.some((item) => isActive(item.href)) ? 'text-primary' : 'text-muted-foreground'}`}><MoreHorizontal className="h-5 w-5" /><span>More</span></button></div>
      ...
      <main className={`flex h-full min-h-0 flex-col pt-[--header-height] pb-[calc(var(--bottom-bar-height)+env(safe-area-inset-bottom,0px))] transition-[padding-left] duration-[--duration-normal] ${collapsed ? 'lg:pl-[--sidebar-collapsed]' : 'lg:pl-[--sidebar-width]'} lg:pt-0 lg:pb-0`}>
        <header className="apple-chrome hidden min-h-[--header-height] items-center justify-between border-b px-6 lg:flex xl:px-8">
          <div className="flex items-center gap-3">{collapsed && <button onClick={toggleCollapse} className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-full text-muted-foreground hover:bg-surface-2" aria-label="Expand sidebar"><Menu className="h-4 w-4" /></button>}<div><p className="caption-label text-muted-foreground">Student workspace</p><h1 className="font-heading text-xl font-bold tracking-tight">{pageTitle}</h1></div></div>
