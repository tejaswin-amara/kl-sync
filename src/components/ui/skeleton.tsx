...
function Skeleton({
  ...
  return (
    <div
      className={cn(
        'bg-surface-2 rounded-[--radius-md]',
        ...
        variant === 'card' && 'h-32 w-full rounded-xl border border-border/50 p-4',
...
function SkeletonCard({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        'p-5 rounded-xl border border-border/60 bg-surface-1/60 space-y-4 animate-pulse',
        className
      )}
      {...props}
    >
...
function SkeletonTable({ rows = 4, className }: { rows?: number; className?: string }) {
  return (
    <div className={cn('w-full space-y-3', className)}>
      <Skeleton variant="table-row" className="h-10 bg-surface-3/50" />
      {Array.from({ length: rows }).map((_, i) => (
        <Skeleton key={i} variant="table-row" className="h-12 opacity-80" />
      ))}
    </div>
  );
}
