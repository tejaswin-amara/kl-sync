...
export async function GET(request: Request) {
  ...
  if (id && !/^[a-zA-Z0-9]+$/.test(id)) return new NextResponse('Invalid ID format', { status: 400 });
  if (path && (path.includes('..') || /%2e/i.test(path) || path.includes('://') || path.includes('//'))) {
  ...
  if (path) {
    ...
    targetRelativePaths.push(`/uploads/studentphotos/${encodeURIComponent(id)}.jpg`, `/uploads/StudentPhotos/${encodeURIComponent(id)}.jpg`);
  }
  ...
  try {
    ...
    if (!ALLOWED_IMAGE_TYPES.has(contentType)) return new NextResponse('Unsupported photo type', { status: 415 });
    ...
    console.error('[PHOTO] Upstream fetch failed:', error instanceof Error ? error.message : 'Unknown error');
