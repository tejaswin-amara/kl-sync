...
function postProcessOCRText(text: string): string {
  ...
  processed = processed.replace(/23C[\/\|l1I\s](\d{4})/g, '23CI$1') // 23C/2001 -> 23CI2001
  processed = processed.replace(/23SC[\/\|l1I\s](\d{4})/g, '23SC$1') // 23SC/3201 -> 23SC3201
  processed = processed.replace(/23MT[\/\|l1I\s](\d{4})/g, '23MT$1') // 23MT/2004 -> 23MT2004
  processed = processed.replace(/23CS[\/\|l1I\s](\d{3}[A-Z])/g, '23CS$1') // 23CS/235F -> 23CS235F
  processed = processed.replace(/23SDC[\/\|l1I\s](\d{3}[A-Z])/g, '23SDC$1') // 23SDC/313R -> 23SDC313R
  processed = processed.replace(/23DEA[\/\|l1I\s](\d{3}[A-Z]{2})/g, '23DEA$1') // 23DEA/310TR -> 23DEA310TR
  processed = processed.replace(/23UC[\/\|l1I\s](\d{4})/g, '23UC$1') // 23UC/0009 -> 23UC0009
  processed = processed.replace(/23CC[\/\|l1I\s](\d{4})/g, '23CC$1') // 23CC/3103 -> 23CC3103
