...
function postProcessOCRText(text: string): string {
  ...
  processed = processed.replace(/23C[\/I](\d{4})/g, '23CI$1') // 23C/2001 or 23CI2001 -> 23CI2001
  processed = processed.replace(/23SC[\/I](\d{4})/g, '23SC$1') // 23SC/3201 or 23SCI3201 -> 23SC3201
  processed = processed.replace(/23MT[\/I](\d{4})/g, '23MT$1') // 23MT/2004 or 23MTI2004 -> 23MT2004
  processed = processed.replace(/23CS[\/I](\d{3}[A-Z])/g, '23CS$1') // 23CS/235F or 23CSI235F -> 23CS235F
  processed = processed.replace(/23SDC[\/I](\d{3}[A-Z])/g, '23SDC$1') // 23SDC/313R or 23SDCI313R -> 23SDC313R
  processed = processed.replace(/23DEA[\/I](\d{3}[A-Z]{2})/g, '23DEA$1') // 23DEA/310TR or 23DEAI310TR -> 23DEA310TR
  processed = processed.replace(/23UC[\/I](\d{4})/g, '23UC$1') // 23UC/0009 or 23UCI0009 -> 23UC0009
  processed = processed.replace(/23CC[\/I](\d{4})/g, '23CC$1') // 23CC/3103 or 23CCI3103 -> 23CC3103
