...
import { fetchExamSeatingData, ScraperSession } from '@/lib/scraper';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await fetchExamSeatingData(session);
    return NextResponse.json(result);
  } catch (error: any) {
    console.error('[fetch-exam-seating] Error:', error);
