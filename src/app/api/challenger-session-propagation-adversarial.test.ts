...
import { GET as erpProxyGET, POST as erpProxyPOST } from '@/app/api/erp-proxy/[module]/route';
...
test('Challenger M1 - API /api/captcha session emission', async () => {
  ...
  assert.strictEqual(response.status, 200, 'Captcha GET should return 200 status');
  ...
  assert.ok(emittedSessionId.startsWith('enc.'), 'Emitted session ID must be encrypted standard format');

  const decoded = await decodeSession(emittedSessionId);
  assert.ok(decoded.csrfToken, 'Decoded session must contain a valid csrfToken');
});
...
test('Challenger M1 - API /api/login session propagation (Headers vs Body vs Missing)', async () => {
  ...
  assert.ok(dataHeader.sessionId, 'Login response must return updated sessionId');
  const decodedFromResHeader = await decodeSession(dataHeader.sessionId);
  assert.ok(decodedFromResHeader.cookies, 'Decoded session should contain cookies');
...
test('Challenger M1 - API /api/erp-proxy/[module] session propagation channels', async () => {
  ...
  const reqHeader = new NextRequest('http://localhost:3000/api/erp-proxy/attendance?academicYear=2025-2026&semesterId=1', {
    method: 'GET',
  ...
  const reqCookie = new NextRequest('http://localhost:3000/api/erp-proxy/attendance?academicYear=2025-2026&semesterId=1', {
    method: 'GET',
  ...
  const reqQuery = new NextRequest(`http://localhost:3000/api/erp-proxy/attendance?sessionId=${encodeURIComponent(token)}&academicYear=2025-2026&semesterId=1`, {
    method: 'GET',
  });
...
test('Challenger M1 - API /api/fetch-photo session extraction', async () => {
  ...
  const reqNoSession = new NextRequest('http://localhost:3000/api/fetch-photo?id=2100030000');
  ...
  const reqHeader = new NextRequest('http://localhost:3000/api/fetch-photo?id=2100030000', {
    headers: { 'x-session-id': token },
  });
  ...
  assert.notStrictEqual(resHeader.status, 401, 'Request with valid session should pass auth check');
});
...
test('Challenger M1 - Multi-Hop Session Round-Trip Chain Simulation', async () => {
  ...
  const erpReq = new NextRequest('http://localhost:3000/api/erp-proxy/attendance', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({
      sessionId: tokenS2,
  ...
  const erpRes = await erpProxyPOST(erpReq, { params: Promise.resolve({ module: 'attendance' }) });
  assert.strictEqual(erpRes.status, 200);

  // Step 4: GET /api/fetch-photo with token S2 in header
  const photoReq = new NextRequest('http://localhost:3000/api/fetch-photo?id=2100030000', {
    headers: { 'x-session-id': tokenS2 },
  });
  const photoRes = await fetchPhotoGET(photoReq);
  assert.notStrictEqual(photoRes.status, 401, 'Photo request with S2 header passed auth');
