                    ...
                    return (
                      <tr key={idx} className={`hover:bg-white/4 transition-colors ${unpaid ? 'bg-destructive/5' : ''}`}>
                        {Object.entries(row).map(([key, val], j) => {
                          ...
                          return (
                            <td key={j} className="px-5 py-3.5 text-sm text-foreground tabular-numbers font-medium whitespace-nowrap">
                              {isStatus ? (
                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold apple-pill ${
                                  unpaid ? 'bg-destructive/15 text-destructive border border-destructive/25' : 'bg-success/15 text-success border border-success/25'
                                }`}>
