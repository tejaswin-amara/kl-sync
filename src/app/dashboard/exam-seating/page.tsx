      ...
      <div className="card bg-zinc-900/40 backdrop-blur-xl border border-white/5 overflow-hidden min-h-[400px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-[400px] text-center">
            ...
            <p className="text-base text-zinc-400 text-gray-400">No exam seating data found.</p>
          </div>
        ) : (
                  ...
                  <tr key={idx} className="group transition-all">
                    {Object.values(row).map((val: any, j) => {
                      ...
                      if (colName.includes('seat')) {
                         displayVal = <span className="inline-flex items-center gap-1 bg-indigo-500/10 text-indigo-400 px-3 py-1 rounded text-sm font-bold uppercase tracking-wider border border-[var(--color-primary)]/20"><span className="material-symbols-outlined text-[16px]">chair_alt</span>{val}</span>;
                      }
