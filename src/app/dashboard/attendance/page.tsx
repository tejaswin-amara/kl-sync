      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-base text-zinc-400 text-gray-400 mt-1">
            Real-time attendance fetched directly from the ERP.
          </p>
        </div>
          ...
          <div className="p-6">
            ...
            <div className="flex flex-col gap-4">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="h-12 w-full bg-zinc-800/30 rounded-lg animate-pulse"
                ></div>
              ))}
            </div>
          </div>
                      ...
                      if (typeof val === 'string' && val.includes('%')) {
                        ...
                        if (!isNaN(num)) {
                          ...
                          if (total > 0) {
                            if (num < 85) {
                              const needed = Math.ceil(
                                (0.85 * total - attended) / 0.15
                              );
                              if (needed > 0)
                                projection = (
                                  <span className="block mt-1 text-[10px] text-zinc-400 font-mono tracking-wider">
                                    Need {needed} classes
                                  </span>
                                );
                            } else if (num >= 85) {
                              const skip = Math.floor(
                                (attended - 0.85 * total) / 0.85
                              );
                              if (skip > 0)
                                projection = (
                                  <span className="block mt-1 text-[10px] text-zinc-400 font-mono tracking-wider">
                          ...
                          if (num >= 85) {
                            displayVal = (
                              <div>
                                <span className="inline-flex items-center gap-1 bg-green-500/10 text-green-400 px-2 py-1 rounded text-xs font-bold">
                            ...
                            displayVal = (
                              <div>
                                <span className="inline-flex items-center gap-1 bg-yellow-500/10 text-yellow-400 px-2 py-1 rounded text-xs font-bold">
                            ...
                            displayVal = (
                              <div>
                                <span className="inline-flex items-center gap-1 bg-red-500/10 text-red-400 px-2 py-1 rounded text-xs font-bold">
