import { getCaptcha, loginAndFetchSemesters, fetchAttendanceData } from '../src/lib/scrapers/attendance';
...
async function diagnose() {
  try {
    ...
    let solvedCaptcha = ocrText.trim().toLowerCase().replace(/[^a-z]/g, '');
    ...
    if (!solvedCaptcha || solvedCaptcha.length < 3) {
      ...
      const solved2 = ocrText2.trim().toLowerCase().replace(/[^a-z]/g, '');
    ...
    if (loginResult.success && loginResult.academicYears.length > 0) {
      ...
      const attendanceResult = await fetchAttendanceData(loginResult.session, loginResult.csrfToken, year, sem);
